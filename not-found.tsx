...
import { areas, events, routes, spots, stories } from "../src/content";
...
for (const [type, items] of Object.entries({ areas, routes, spots, stories, events })) {
...
for (const item of [...routes, ...spots, ...stories]) {
...
console.log(`Content validation passed: ${areas.length} areas, ${routes.length} routes, ${spots.length} spots, ${stories.length} stories, ${events.length} current events.`);
